...
class NukeEditor(ToolPane):
    ...
    BT_LIMITS = wx.NewIdRef()
    BT_NEUTRAL = wx.NewIdRef()
    BT_SIGN = wx.NewIdRef()
    BT_TUNER = wx.NewIdRef()
    BT_EXPORT = wx.NewIdRef()
    BT_DRIVE = wx.NewIdRef()
    ID_IKTYPE = wx.NewIdRef()
    ID_IKOPT = wx.NewIdRef()
    ID_GAIT_BOX = wx.NewIdRef()
    ID_ANY = wx.NewIdRef()
    ...
    def __init__(self, parent, port=None):
        ...
        self.ikType = wx.ComboBox(
            self, self.ID_IKTYPE, choices=[*iKmodels])
        ...
        configSizer.Add(self.optLabel, (1, 0), wx.GBSpan(
        ...
        actionSizer.Add(wx.StaticText(self, -1, "Capture Limits:"),
                        (0, 0), wx.GBSpan(1, 1), wx.ALIGN_CENTER_VERTICAL)
        actionSizer.Add(wx.StaticText(self, -1, "Capture Neutral:"),
                        (1, 0), wx.GBSpan(1, 1), wx.ALIGN_CENTER_VERTICAL)
        actionSizer.Add(wx.StaticText(self, -1, "Set/Test Signs:"),
                        (2, 0), wx.GBSpan(1, 1), wx.ALIGN_CENTER_VERTICAL)
        ...
        # event handling
        self.Bind(wx.EVT_BUTTON, self.doLimits, self.BT_LIMITS)
        self.Bind(wx.EVT_BUTTON, self.doNeutral, self.BT_NEUTRAL)
        self.Bind(wx.EVT_BUTTON, self.doSignTest, self.BT_SIGN)
    ...
    def getModel(self):
        ...
        # make instance
        # dofORlegs/debug/GaitGen
        self.model = modelClass(int(self.optChoice), True)
        return self.model
    ...
    def makePanel(self):
        ...
        for group in self.model.vars_layout:
            ...
            bodySizer.Add(wx.StaticText(
                self, -1, group[0]), (index, 0), wx.GBSpan(1, 1), wx.EXPAND | wx.TOP, 10)
            index = index + 1
            count = 0
            for field in group[1:-1]:
                d = self.model.vars[field]
                bodySizer.Add(wx.StaticText(
                    self, -1, d[0]), (index, 0), wx.GBSpan(1, 1), wx.ALIGN_CENTER_VERTICAL)
                self.vars.append(wx.SpinCtrl(
                    self, self.ID_ANY, str(d[1]), min=-5000, max=5000))
                bodySizer.Add(self.vars[-1], (index, 1))
                index = index + 1
                count = count + 1
            if group[-1] != "":
                picture = wx.StaticBitmap(self, bitmap=wx.Bitmap(
                    "tools/models/" + iKmodels[self.ikChoice].folder + "/" + group[-1]))
        ...
        for servo_name in self.model.servo_layout:
            if servo_name != "":
                servoSizer.Add(wx.StaticText(self, -1, servo_name), (index / 2,
                                                                     (index % 2) * 2), wx.GBSpan(1, 1), wx.ALIGN_CENTER_VERTICAL)
                new_servo = wx.SpinCtrl(self, self.ID_ANY, str(
                    self.model.servos[servo_name]), min=1, max=self.parent.project.count)
                self.servos.append(new_servo)
                servoSizer.Add(new_servo, (index / 2, (index % 2)
                                           * 2 + 1), wx.GBSpan(1, 1), wx.EXPAND)
            index = index + 1
    ...
    def loadData(self):
        ...
        if self.parent.project.name == "":
            ...
            if self.ikType.GetValue() == "":
                ...
                self.ikOpt.SetItems(
                    [str(c) for c in iKmodels[self.ikType.GetValue()].options])
                self.ikOpt.Enable()
    ...
    def save(self, e=None):
        ...
        if self.ikType.GetValue() == "" or self.ikOpt.GetValue() == "":
            return
        nukeStr = self.ikChoice + "," + \
            str(self.ikOpt.GetValue()) + "," + self.signs + ","
        # vars
        for var in self.vars:
    ...
    def configModel(self):
        ...
        if "tools/models/" + modelClassName not in sys.path:
            sys.path.append("tools/models/" + modelClassName)
        modelModule = __import__(
            modelClassName, globals(), locals(), [modelClassName])
        modelClass = getattr(modelModule, modelClassName)
        # make instance
        # dofORlegs/debug/GaitGen
        model = modelClass(int(self.optChoice), True)
        model.config(self.optChoice, [int(v.GetValue()) for v in self.vars], [int(
            s.GetValue()) for s in self.servos], [1024, ] + self.parent.project.resolution)
        ...
        if servoID <= self.parent.project.count:
            if end == 0:
                ...
                if error == 0:      # first time through, ask user to prepare
                    dlg = NukeDialog(self.parent, 'Capture Limits',
                                     'Click OK when you have moved\n   servo ' + str(servoID) + ' to one extreme')
                    result = dlg.ShowModal()
                    dlg.Destroy()
                if result == wx.ID_OK:
                    try:
                        # read in servo position
                        a = self.port.getReg(servoID, ax12.P_PRESENT_POSITION_L, 2)
                        a = a[0] + (a[1] << 8)
                        # get other limit
                        ...
                        if error < 3:   # try again
                            ...
                            dlg = wx.MessageDialog(
                                self.parent, 'Unable to read servo ' + str(servoID), 'Capture Error', wx.OK)
                            dlg.ShowModal()
    ...
    def doNeutral(self, e=None):
        ...
        if self.doChecks(["project", "port"]) > 0:
            ...
            dlg = NeutralDialog(self.parent, 'Capture Neutral Position',
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for servo in range(self.parent.project.count):
                    ...
                    if pos != -1:
                        self.parent.project.poses["ik_neutral"][servo] = pos[0] + (
                            pos[1] << 8)
                    else:
    ...
    def doExport(self, e=None):
        if self.doChecks(["project", "ik"]) == 0:
            ...
            for k, v in servoMap.items():
                params["@SERVO_INDEXES"] = params["@SERVO_INDEXES"] + \
                    "#define " + k + " " + str(v) + "\n"
            params["@SERVO_MINS"] = "int mins[] = {" + \
                str(self.parent.project.poses["ik_min"]) + "};"
            params["@SERVO_MAXS"] = "int maxs[] = {" + \
                str(self.parent.project.poses["ik_max"]) + "};"
            # Simple Heuristics (TODO: Replace with a gait builder)
            params["@X_STANCE"] = str(self.vars[0].GetValue())
            params["@Y_STANCE"] = str(
                self.vars[0].GetValue() + self.vars[1].GetValue())
            ...
            template = open("tools/models/" + modelDir +
                            "/template.ik").readlines()
            current = ""
            for line in template:
                if line.find("@") == 0 and current == "":
                    current = line.strip().rstrip()
            ...
            templates[sketch +
                      ".ino"] = open("tools/models/core/default.pde").read()
            # for each file
            for fileName in templates.keys():
                ...
                for var, val in params.items():
                    if var.find("@") == 0:
                        templates[fileName] = templates[fileName].replace(
                            var, val)
                for k, v in servoMap.items():
                    templates[fileName] = templates[fileName].replace(
                        "@NEUTRAL_" + k, str(self.parent.project.poses["ik_neutral"][v - 1]))
                    templates[fileName] = templates[fileName].replace(
                        "@SIGN_" + k, self.signs[v - 1:v])
                ...
                while i < len(template):
                    if template[i].find("@IF") >= 0:
                        # do we include this?
                        line = template[i][template[i].find(
                            "@IF") + 3:].strip().rstrip()
                        var = line[0:line.find(" ")].rstrip()
                        val = line[line.find(
                            " ") + 1:].strip().rstrip().split()
                        i = i + 1
...
class NukeDialog(wx.Dialog):
    def __init__(self, parent, title, msg):
        ...
        vbox.Add(wx.StaticText(self, -1, msg, (340, 100)),
                 1, wx.ALIGN_CENTER | wx.BOTTOM | wx.TOP, 10)
        ...
        self.SetSizer(vbox)
        self.Bind(wx.EVT_BUTTON, self.doBackUp, 42)
